...
interface ZodValidationException extends Error {
  getResponse?: () => { issues?: ZodIssueMinimal[]; errors?: ZodIssueMinimal[] };
  zodError?: { issues: ZodIssueMinimal[] };
}
...
export class GlobalExceptionFilter implements ExceptionFilter {
  constructor(
    ...
    const path: string = req?.url ?? 'unknown';
  ...
  private normalizeException(exception: unknown): {
    ...
    if (this.isZodValidationException(exception)) {
      const resBody =
        typeof exception.getResponse === 'function' ? exception.getResponse() : undefined;

      const issues = resBody?.issues ?? resBody?.errors ?? exception.zodError?.issues ?? [];

      const details = Array.isArray(issues)
        ? issues.map((i: ZodIssueMinimal) => ({
    ...
    if (exception instanceof HttpException) {
      ...
      const msg =
        typeof response === 'string'
          ? response
          : ((response as HttpExceptionBody)?.message ?? exception.message);
  ...
  private isZodValidationException(e: unknown): e is ZodValidationException {
    return e instanceof Error && e.name === 'ZodValidationException';
  }
}
