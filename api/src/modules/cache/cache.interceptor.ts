...
import { buildEntityIdCacheKey, resolveEntity, resolveId } from './cache-key.util';
...
function ensureEnvelope<T>(body: unknown): SuccessResponse<T> {
...
function withMeta<T>(resp: SuccessResponse<T>, patch: Record<string, unknown>): SuccessResponse<T> {
  return {
    ...resp,
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    if (isRead && typeof ttlSeconds === 'number' && ttlSeconds > 0) {
      ...
      if (!key) {
        ...
        return next
          .handle()
          .pipe(map((body: unknown) => withMeta(ensureEnvelope(body), { isCached: false })));
      }
        ...
        switchMap((cachedRaw) => {
          ...
          return next.handle().pipe(
            switchMap(async (body: unknown) => {
              ...
              await this.redisService.set(key, JSON.stringify(envelope), { exSeconds: ttlSeconds });
      ...
      return next.handle().pipe(
        tap((body: unknown) => {
          ...
          const id = resolveId(
            ...
            body as { id?: string; ID?: string; data?: { id?: string; ID?: string } } | null,
          );
          if (!id) {
          ...
          void this.redisService.del(cacheKey).then(() => {
            this.logger.info('cache_invalidated', CTX, { key: cacheKey });
          });
        }),
        map((body: unknown) => withMeta(ensureEnvelope(body), { isCached: false })),
      );
    }
